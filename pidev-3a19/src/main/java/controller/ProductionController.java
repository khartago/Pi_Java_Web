...
public class ProductionController {
    ...
    public void initialize() {
        ...
        tableProduction.getSelectionModel().selectedItemProperty().addListener((obs, oldV, newV) -> {
            if (newV != null) {
                ...
                if (newV.getDatePlante() != null) dpDatePlante.setValue(newV.getDatePlante().toLocalDate());
            }
        });
    ...
    private void addAccepterButtonToTable() {
        colAction.setCellFactory(param -> new TableCell<Production, Void>() {
            private final Button btn = new Button("Accepter");
            {
                ...
                btn.setOnAction(event -> {
                    ...
                    if (selected.getEtat() != null && "COMPLETE".equalsIgnoreCase(selected.getEtat())) {
                        showAlert(Alert.AlertType.WARNING, "Deja accepte", "Plantation deja COMPLETE.");
                        return;
                    }
                    ...
                    try {
                        ...
                        productionPlanteService.ajouter(new ProductionPlante((float) selected.getQuantite(),
                                new Date(System.currentTimeMillis()), "Bonne", "Recoltee"));
                        showAlert(Alert.AlertType.INFORMATION, "Succes", "Plantation acceptee.");
                        refreshTable();
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty) setGraphic(null);
                    ...
                    if (p.getEtat() != null && "COMPLETE".equalsIgnoreCase(p.getEtat())) {
                        btn.setDisable(true);
    ...
    public void ajouterProduction() {
        try {
            if (tfNomPlant.getText().isEmpty() || tfVariete.getText().isEmpty() || tfQuantite.getText().isEmpty()
                    || dpDatePlante.getValue() == null || tfSaison.getText().isEmpty()) {
                showAlert(Alert.AlertType.WARNING, "Champs vides", "Remplir tous les champs.");
                return;
            }
            productionService.ajouter(new Production(tfNomPlant.getText(), tfVariete.getText(),
                    Integer.parseInt(tfQuantite.getText()), Date.valueOf(dpDatePlante.getValue()), tfSaison.getText()));
            showAlert(Alert.AlertType.INFORMATION, "Succes", "Production ajoutee.");
            clearForm();
            refreshTable();
        } catch (NumberFormatException e) { showAlert(Alert.AlertType.ERROR, "Erreur", "Quantite invalide."); }
        catch (Exception e) { showAlert(Alert.AlertType.ERROR, "Erreur", e.getMessage()); }
    }
    ...
    public void modifierProduction() {
        try {
            ...
            if (selected == null) { showAlert(Alert.AlertType.WARNING, "Selection", "Selectionner une production."); return; }
            productionService.modifier(new Production(selected.getId(), tfNomPlant.getText(), tfVariete.getText(),
                    Integer.parseInt(tfQuantite.getText()), Date.valueOf(dpDatePlante.getValue()), tfSaison.getText(), selected.getEtat()));
            showAlert(Alert.AlertType.INFORMATION, "Succes", "Production modifiee.");
            clearForm();
            refreshTable();
        } catch (Exception e) { showAlert(Alert.AlertType.ERROR, "Erreur", e.getMessage()); }
    }
    ...
    public void supprimerProduction() {
        try {
            ...
            showAlert(Alert.AlertType.INFORMATION, "Succes", "Production supprimee.");
            clearForm();
            refreshTable();
        } catch (Exception e) { showAlert(Alert.AlertType.ERROR, "Erreur", e.getMessage()); }
    }
