...
public class MesFavorisController {
    ...
    private void loadFavoris() {
        ...
        // Mettre ├á jour le titre
        if (titleLabel != null) {
            titleLabel.setText("ÔØñ´©Å Mes Favoris (" + favoris.size() + ")");
        }
        ...
        if (favoris.isEmpty()) {
            ...
            if (emptyLabel != null) {
                ...
                emptyLabel.setText("Aucun favori pour le moment.\nExplorez le Marketplace et ajoutez des produits ├á vos favoris! ­ƒÿè");
            }
            if (cardsPane != null) {
    ...
    private VBox createCard(Produit p) {
        ...
        // Bouton D├®tails
        Button detailsButton = new Button("­ƒæü´©Å D├®tails");
        ...
        Button removeButton = new Button("­ƒÆö Retirer des favoris");
    ...
    private void removeFavoris(Produit p) {
        ...
        confirm.setHeaderText("├ètes-vous s├╗r?");
        ...
        if (confirm.showAndWait().get() == javafx.scene.control.ButtonType.OK) {
            if (favorisDAO.removeFavoris(p.getIdProduit())) {
                ...
                loadFavoris(); // Rafra├«chir la liste
            } else {
