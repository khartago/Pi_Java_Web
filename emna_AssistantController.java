 ...
 * Le r├®sultat affiche une fiche produit + liste des mat├®riels associ├®s.
 */
public class AssistantController {
    ...
    private void initialize() {
        ...
        // UI defaults
        selectedImageLabel.setText("Aucune image s├®lectionn├®e");
    ...
    private void handleBack() {
        try {
            ...
            e.printStackTrace();
            showError("Navigation", "Impossible de retourner ├á la liste des produits.");
        }
    }
    ...
    private void handleSearchByName() {
        ...
        if (results.isEmpty()) {
            ...
            showWarning("Aucun r├®sultat", "Aucun produit ne correspond ├á: " + q);
            return;
        }
    ...
    private void handleAnalyzeImage() {
        ...
        if (all.isEmpty()) {
            showWarning("Base vide", "Aucun produit trouv├® dans la base.");
            return;
        }
        ...
        for (Produit p : all) {
            ...
            // 1) M├¬me chemin (souvent le cas si lÔÇÖutilisateur choisit exactement la m├¬me image)
            ...
            // 4) Similarit├® (fallback l├®ger)
            int sim = simpleSimilarityScore(targetName, normalizeText(p.getNom()));
            if (sim > 0) {
                score += sim;
                reasons.add("Similarit├® nom (fallback)");
            }

            if (score > 0) {
                scored.add(new Scored(p, score, String.join(" ÔÇó ", reasons)));
            }
        }
        ...
        if (scored.isEmpty()) {
            clearDetails();
            resultsTable.setItems(FXCollections.observableArrayList());
            showWarning("Aucun match", "Impossible dÔÇÖidentifier le produit depuis lÔÇÖimage. Essayez avec le nom du produit.");
            return;
        }
    ...
    private void showProduitDetails(Produit p) {
        ...
        // Charge lÔÇÖimage (comme MarketplaceController)
