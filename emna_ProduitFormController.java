...
public class ProduitFormController {
    ...
    public void setProduit(Produit produit) {
        ...
        if (produit != null) {
            ...
            // Ô£à NOUVEAU : afficher l'image en mode modification
    ...
    private void handleSave() {
        ...
        // Ô£à NOUVEAU : sauvegarder imagePath
        ...
        // Ô£à G├®n├®ration du QR code (best effort)
        try {
            Path qrPath = QrCodeService.generateProduitQr(produit.getIdProduit());
            System.out.println("QR code g├®n├®r├® pour le produit " + produit.getIdProduit() + " : " + qrPath);
        } catch (IOException e) {
            e.printStackTrace();
            // On ne bloque pas l'enregistrement pour une erreur de QR
        }

        // Ô£à Tra├ºabilit├® : enregistrer l'├®v├®nement
        ...
        h.setCommentaire(isNew ? "Cr├®ation du produit" : "Mise ├á jour du produit");
    ...
    // Ô£à NOUVEAU : bouton "Choisir" (FileChooser)
    @FXML
    ...
    private boolean validateInput() {
        ...
        if (nom == null || nom.trim().isEmpty()) {
            ...
            if (nom.length() < 2 || nom.length() > 50) {
                errors.append("- Le nom doit contenir entre 2 et 50 caract├¿res.\n");
            }
            if (!nom.matches("[a-zA-Z├Ç-├┐0-9 ]+")) {
        ...
        if (quantiteText == null || quantiteText.trim().isEmpty()) {
            errors.append("- La quantit├® est requise.\n");
        } else {
            try {
                int qty = Integer.parseInt(quantiteText.trim());
                if (qty < 0) errors.append("- La quantit├® doit ├¬tre positive.\n");
            } catch (NumberFormatException e) {
                errors.append("- La quantit├® doit ├¬tre un nombre entier.\n");
            }
        }
        ...
        if (unite == null || unite.trim().isEmpty()) {
            errors.append("- L'unit├® est requise.\n");
        } else {
            unite = unite.trim().toLowerCase();

            if (!(unite.equals("kg") || unite.equals("l") || unite.equals("piece"))) {
                errors.append("- L'unit├® doit ├¬tre : kg, l ou piece uniquement.\n");
            }
        }

        // (optionnel) validation image : si rempli, v├®rifier extension
        String img = imagePathField.getText();
        if (img != null && !img.trim().isEmpty()) {
            String lower = img.trim().toLowerCase();
            if (!(lower.endsWith(".png") || lower.endsWith(".jpg") || lower.endsWith(".jpeg"))) {
                errors.append("- L'image doit ├¬tre au format png/jpg/jpeg.\n");
            }
        }
