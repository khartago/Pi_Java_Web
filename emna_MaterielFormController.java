...
public class MaterielFormController {
    ...
    private void initialize() {
        ...
        // Bloquer toutes les dates pass├®es (cr├®ation + modification)
    ...
    private void handleSave() {
        ...
        if (!success) {
            showError("Impossible d'enregistrer le mat├®riel.");
        } else {
            closeWindow();
        }
    }
    ...
    private boolean validateInput() {
        ...
        if (nom == null || nom.trim().isEmpty()) {
            errors.append("- Le nom du mat├®riel est requis.\n");
        } else {
            nom = nom.trim();
        ...
        if (etat == null || etat.trim().isEmpty()) {
            errors.append("- L'├®tat du mat├®riel est requis.\n");
        } else {
            etat = etat.trim().toLowerCase();

            if (!(etat.equals("neuf") || etat.equals("usag├®") || etat.equals("bon"))) {
                errors.append("- L'├®tat doit ├¬tre : neuf, usag├® ou bon.\n");
            }
        }
        ...
        if (selectedDate == null) {
            ...
            errors.append("- La date d'achat doit ├¬tre aujourdÔÇÖhui ou future.\n");
        }

        if (coutField.getText() == null || coutField.getText().trim().isEmpty()) {
            errors.append("- Le co├╗t est requis.\n");
        } else {
            try {
                double cost = Double.parseDouble(coutField.getText().trim());
                if (cost < 0) errors.append("- Le co├╗t doit ├¬tre positif.\n");
            } catch (NumberFormatException e) {
                errors.append("- Le co├╗t doit ├¬tre un nombre valide.\n");
            }
        }
