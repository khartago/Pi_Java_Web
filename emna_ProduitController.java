...
public class ProduitController {
    ...
    private void handleModifier() {
        ...
        if (selected != null) {
            ...
            showWarning("S├®lection requise", "Veuillez s├®lectionner un produit ├á modifier.");
        }
    }
    ...
    private void handleSupprimer() {
        Produit selected = produitTable.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showWarning("S├®lection requise", "Veuillez s├®lectionner un produit ├á supprimer.");
            return;
        }
        ...
        confirm.setContentText("├ètes-vous s├╗r de vouloir supprimer le produit " + selected.getNom() + " ?");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            if (produitDAO.delete(selected.getIdProduit())) {
                ...
                showError("Erreur lors de la suppression",
                        "Impossible de supprimer ce produit. Il est peut-├¬tre r├®f├®renc├® par un mat├®riel.");
            }
        }
    }
    ...
    private void handleVoirMateriels() {
        Produit selected = produitTable.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showWarning("S├®lection requise", "Veuillez s├®lectionner un produit pour voir ses mat├®riels.");
            return;
        }
        ...
        try {
            ...
            stage.setTitle("Mat├®riels de " + selected.getNom());
            ...
            e.printStackTrace();
            showError("Erreur", "Impossible d'ouvrir la liste des mat├®riels.");
        }
    }
    ...
    private void handleOpenAssistant() {
        try {
            ...
            Stage stage = (Stage) produitTable.getScene().getWindow();
            stage.setScene(scene);
            stage.setTitle("Assistant IA - D├®tails produit");
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void handleTraceabilite() {
        Produit selected = produitTable.getSelectionModel().getSelectedItem();
        if (selected == null) {
            showWarning("S├®lection requise", "Veuillez s├®lectionner un produit pour voir sa tra├ºabilit├®.");
            return;
        }
        ...
        try {
            ...
            e.printStackTrace();
            showError("Erreur", "Impossible d'ouvrir la tra├ºabilit├®.");
        }
    }
    ...
    private void handleNotifierExpiration() {
        try {
            // Option 1: Utiliser une bo├«te de dialogue pour entrer l'email
            ...
            // Configuration email (├Ç personnaliser)
            String sender = "amnafati94@gmail.com";  // Ô£à Votre email d'envoi
            String appPassword = "jcdo lljy fgug omgr";  // Ô£à Votre mot de passe d'application

            // ÔÜá´©Å MEILLEURE PRATIQUE: Utiliser des variables d'environnement au lieu de coder en dur
            ...
            alert.setHeaderText("R├®sultat envoi email");
            alert.setContentText(total == 0
                    ? "Aucune expiration ├á signaler."
                    : "Email envoy├® ├á " + recipient + ".\nProduits concern├®s: " + total);
            alert.showAndWait();

        } catch (Exception e) {
            e.printStackTrace();
            showError("Envoi email ├®chou├®", e.getMessage());
        }
    }
